...
pub struct NoteEditing {
    ...
    //note_drag_base_tick_and_note: (SignedMIDITick, SignedMIDITick),
    note_drag_note_pos: (MIDITick, u8),
    note_drag_start_pos: (MIDITick, u8),
...
impl NoteEditing {
    pub fn new(
        ...
        Self {
            ...
            //drag_offset: 0,
            //note_drag_base_tick_and_note: (0, 0),
            note_drag_start_pos: (0, 0),
            note_drag_note_pos: (0, 0),
    ...
    pub fn on_mouse_down(&mut self) {
        ...
        match editor_tool {
            EditorTool::Pencil => {
                ...
                //self.drag_offset = 0;
                //self.note_drag_base_tick_and_note = (0, 0);
                self.note_drag_start_pos = (0, 0);
                ...
                if self.flags & NOTE_EDIT_IS_EDITING != 0 {
                    ...
                    if self.flags & NOTE_EDIT_MULTIEDIT == 0 {
                        if !ghost_notes.is_empty() {
                            let gn = ghost_notes[0].get_note_mut();
                            gn.start = {
                                let snapped = self.note_drag_note_pos.0 as SignedMIDITick + self.snap_tick(self.curr_mouse_midi_pos.0 as SignedMIDITick - self.note_drag_start_pos.0 as SignedMIDITick);
                                if snapped < 0 { 0 }
                                else { snapped as MIDITick }
                                // let mut snapped = self.note_drag_start_pos.0 as SignedMIDITick + self.snap_tick(self.curr_mouse_midi_pos.0 as SignedMIDITick - self.note_drag_start_pos.0 as SignedMIDITick);
                                //snapped as MIDITick
                            };

                            gn.key = {
                                let key = self.curr_mouse_midi_pos.1;
                                if key > 127 { 127 }
                                else { key }
                            };
                            ...
                            /*gn.start = {
                                let mut snapped =
                                    //self.drag_offset + self.snap_tick(self.curr_mouse_midi_pos.0 as SignedMIDITick);
                                    self.note_drag_base_tick_and_note.1 + self.snap_tick(self.curr_mouse_midi_pos_rounded.0 as SignedMIDITick - self.note_drag_base_tick_and_note.0);
                                if snapped < 0 {
                                    snapped = 0;
                                }
                                snapped
                    ...
                    // multi-note editing
                        ...
                        let base_key = self.note_drag_note_pos.1;

                        for (i, ghost_note) in ghost_notes.iter_mut().enumerate() {
                            let (offs_tick, offs_key) = {
                                let (old_tick, old_key) = self.note_old_positions[i];
                                let tick = base_start as SignedMIDITick - old_tick as SignedMIDITick;
                                let key = base_key as i8 - old_key as i8;
                                (tick, key)
                            };

                            let gn = ghost_note.get_note_mut();
                            (*(*gn).start_mut()) = {
                                let start = base_start as SignedMIDITick + offs_tick;
                                if start < 0 { 0 }
                                else { start as MIDITick }
                            };

                            (*(*gn).key_mut()) = {
                                let key = base_key as i8;
                                key.saturating_add(offs_key).max(0)
                            } as u8;
                        }
                        ...
                        let base_start = {
                            let mut snapped =
                                //self.drag_offset + self.snap_tick(self.curr_mouse_midi_pos.0 as SignedMIDITick - self.drag_offset as SignedMIDITick);
                                self.note_drag_base_tick_and_note.1 + self.snap_tick(self.curr_mouse_midi_pos_rounded.0 as SignedMIDITick - self.note_drag_base_tick_and_note.0);
                            if snapped < 0 {
                                snapped = 0;
                            }
                    ...
                    for (i, id) in self.note_temp_mod_ids.iter().enumerate() {
                        ...
                        let mut new_note_end = self.snap_tick(self.curr_mouse_midi_pos.0 as SignedMIDITick - self.note_drag_base_tick_and_note.0) + old_length as SignedMIDITick;
...
*/
    ...
    fn extract_notes(&mut self, ids: Vec<usize>, track: u16) -> (Vec<usize>, Vec<Note>) {
        let ids_set: HashSet<usize> = ids.into_iter().collect();

        let old_notes = {
            let mut notes = self.notes.write().unwrap();
            std::mem::take(&mut notes[track as usize])
        };

        let mut new_notes = Vec::with_capacity(old_notes.len() - ids_set.len());
        let mut extracted = Vec::with_capacity(ids_set.len());
        for (idx, note) in old_notes.into_iter().enumerate() {
            if ids_set.contains(&idx) {
                extracted.push(note);
            } else {
                new_notes.push(note);
            }
        ...
        let mut ids: Vec<usize> = ids_set.into_iter().collect();
        ids.sort_unstable();
    ...
    fn duplicate_notes(&mut self, note_ids: Vec<usize>, paste_tick: MIDITick, track_src: u16, track_dst: u16, select_duplicate: bool) {
        // 1. collect notes to copy
        let notes_to_copy = {
            let notes = self.notes.read().unwrap();
            let track_src = &notes[track_src as usize];

            let first_note = &track_src[note_ids[0]];

            let mut copied_notes = Vec::new();
            for &id in note_ids.iter() {
                let mut note_copy = track_src[id].clone();
                *(note_copy.start_mut()) = note_copy.start() - first_note.start() + paste_tick;
                copied_notes.push(note_copy);
            }
        ...
        let notes_dst = &mut notes[track_dst as usize];
        let notes_dst_old = std::mem::take(notes_dst);

        let mut notes_dst_iter = notes_dst_old.into_iter().peekable();
        let mut notes_copy_iter = notes_to_copy.into_iter().peekable();

        let mut merged: Vec<Note> = Vec::with_capacity(
            notes_dst_iter.size_hint().0 + notes_copy_iter.size_hint().0
        );

        let mut paste_ids = Vec::new();
        let mut paste_idx = 0;
        loop {
            match (notes_dst_iter.peek(), notes_copy_iter.peek()) {
                (Some(track_note), Some(copy_note)) => {
                    if track_note.start <= copy_note.start {
                        let nval = notes_dst_iter.next().unwrap();
                        merged.push(nval);
                        paste_idx += 1;
                    } else {
                        let cval = notes_copy_iter.next().unwrap();
                        merged.push(cval);
                        paste_ids.push(paste_idx);
                        paste_idx += 1;
                    }
                }
                (Some(_), None) => {
                    merged.extend(notes_dst_iter.by_ref());
                    break;
                }
                (None, Some(_)) => {
                    while let Some(copy_note) = notes_copy_iter.next() {
                        merged.push(copy_note);
                        paste_ids.push(paste_idx);
                        paste_idx += 1;
                    }
                    break;
        ...
        notes[track_dst as usize] = merged;

        {
            let mut sel_ids = self.selected_notes_ids.lock().unwrap();
            (*sel_ids) = paste_ids.clone();
        }
    ...
    fn setup_notes_for_length_change(&mut self, note_ids: Option<Vec<usize>>, curr_track: u16, base_id: usize) {
        ...
        /*self.note_drag_base_tick_and_note = {
            let base_note = &notes[base_id];
            (self.curr_mouse_midi_pos_rounded.0 as SignedMIDITick, base_note.start() as SignedMIDITick)
        };*/
        self.note_drag_start_pos = self.curr_mouse_midi_pos;
    ...
    fn setup_notes_for_drag(&mut self, note_ids: Option<Vec<usize>>, curr_track: u16, base_id: usize) {
        ...
        /*self.drag_offset = {
           ...
           // self.curr_mouse_midi_pos.0 as SignedMIDITick - base_note.start() as SignedMIDITick
            base_note.start() as SignedMIDITick
        };*/
        ...
        self.note_drag_start_pos = self.curr_mouse_midi_pos;
        self.note_drag_note_pos = {
            let notes = self.notes.read().unwrap();
            let notes = &notes[curr_track as usize];
            let base_note = &notes[base_id];
            (base_note.start(), base_note.key())
        };
    ...
    pub fn apply_action(&mut self, action: &mut EditorAction) {
        match action {
            EditorAction::PlaceNotes(note_ids, notes_deleted, track) => {
                ...
                let mut old_notes_iter = old_notes.into_iter().peekable();
                let mut rec_notes_iter = recovered_notes.into_iter().peekable();

                let mut merged = Vec::with_capacity(
                    old_notes_iter.size_hint().0 + rec_notes_iter.size_hint().0
                );

                loop {
                    match (old_notes_iter.peek(), rec_notes_iter.peek()) {
                        (Some(old_note), Some(rec_note)) => {
                            if old_note.start() <= rec_note.start() {
                                let note = old_notes_iter.next().unwrap();
                                merged.push(note);
                            } else {
                                let note = rec_notes_iter.next().unwrap();
                                merged.push(note);
                            }
                        }
                        (Some(_), None) => {
                            merged.extend(old_notes_iter.by_ref());
                            break;
                        }
                        (None, Some(_)) => {
                            merged.extend(rec_notes_iter.by_ref());
                            break;
                        }
                        (None, None) => break
                    }
            ...
            EditorAction::DeleteNotes(note_ids, notes_deleted, track) => {
                ...
                let ids_set: HashSet<usize> = note_ids.into_iter().collect();
                ...
                let mut new_notes = Vec::with_capacity(old_notes.len() - ids_set.len());
                let mut deleted_notes = Vec::with_capacity(ids_set.len());
